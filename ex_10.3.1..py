...
if __name__ == "__main__":
    lst = construct()
    with open('output01.txt', 'w', encoding='utf-8') as f:
        for obj in lst:
            if isIterable(obj):
                f.write(f"Об'єкт типу {type(obj)} є ітерабельним:\n")
                try:
                    for it in obj:
                        f.write(f"  {it}\n")
                except Exception as e:
                    f.write(f"  Помилка ітерації: {str(e)}\n")
            else:
                f.write(f"Об'єкт типу {type(obj)} НЕ є ітерабельним\n")